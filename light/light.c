...
LightThread(void *arg)
{
	...
	if (!faces_sup)
	    LightFace(bsp->dfaces + facenum, NULL, &modelinfo[i], ctx);
	else if (scaledonly)
	{
	    bsp->dfaces[facenum].lightofs = -1;
	    bsp->dfaces[facenum].styles[0] = 255;
	    LightFace(bsp->dfaces + facenum, faces_sup + facenum, &modelinfo[i], ctx);
    }
	else if (faces_sup[facenum].lmscale == modelinfo[i].lightmapscale)
	{
	    LightFace(bsp->dfaces + facenum, NULL, &modelinfo[i], ctx);
...
main(int argc, const char **argv)
{
	...
	printf("usage: light [-threads num] [-extra|-extra4]\n"
	       ...
	       "             [-soft [n]] [-fence] [-gamma n] [-surflight_subdivide n] [-onlyents] [-sunsamples n] [-phongdebug] bspfile\n");
	exit(1);
    }
