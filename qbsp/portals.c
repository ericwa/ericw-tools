...
void WritePortalFile_r (node_t *node)
{
	...
	for (p = node->portals ; p ; )
	{
		...
		if (w && p->nodes[0] == node)
		{
			...
			{	// backwards...
				PortalFile.Printf("%i %i %i ",w->numpoints, p->nodes[1]->visleafnum, p->nodes[0]->visleafnum);
			}
			else
				PortalFile.Printf("%i %i %i ",w->numpoints, p->nodes[0]->visleafnum, p->nodes[1]->visleafnum);
