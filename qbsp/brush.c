...
void CheckFace (face_t *f)
{
	...
	for (i=0 ; i<f->numpoints ; i++)
	{
		...
		d = DotProduct (p1, pPlanes[f->planenum].normal) - pPlanes[f->planenum].dist;
...
=================
*/
int FindTargetEntity(char *szTarget)
{
	int iEntity;
	char *szName;
...
void FixRotateOrigin(int iEntity, vec3_t offset)
{
	...
	sprintf(szOrigin, "%d %d %d", (int)offset[0], (int)offset[1], (int)offset[2]);
...
=============
*/
int AddHullPoint (vec3_t p, vec3_t hull_size[2])
{
	int		i;
	vec_t	*c;
	int		x,y,z;
...
=============
*/
void AddHullEdge (vec3_t p1, vec3_t p2, vec3_t hull_size[2])
{
	int		pt1, pt2;
	int		i;
	int		a, b, c, d, e;
...
=============
*/
void ExpandBrush (vec3_t hull_size[2], face_t *pFaceList)
{
	int		i, x, s;
	vec3_t	corner;
	face_t	*f;
...
brush_t *LoadBrush (int iBrush)
{
	...
	szName = rgszMiptex[pWorldEnt->pTexinfo[map.rgFaces[map.rgBrushes[iBrush].iFaceStart].texinfo].miptex];
...
// no seperate textures on clip hull
	...
	// create the faces
	numbrushfaces = map.rgBrushes[iBrush].iFaceEnd - map.rgBrushes[iBrush].iFaceStart;
	memcpy(faces, &(map.rgFaces[map.rgBrushes[iBrush].iFaceStart]), numbrushfaces * sizeof(mapface_t));
